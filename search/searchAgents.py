...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # second tuple with corners from which i will remove all those i have found
        self.corners_to_pop=self.corners
        # For display purposes
        self.visualize = True
        self._visited, self._visitedlist, self._expanded = {}, [], 0 # DO NOT CHANGE
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        if state[1]:    #has elements to remove
            to_ret=False
        # is the tuple empty=removed all corners from it because we found them
        else:
            to_ret=True
        # For display purposes only
        if to_ret and self.visualize:
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            x,y = state[0]
            have_not_visited=list()
            for i in state[1]:
                have_not_visited.append(i)
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                f=1
                # check if there is in corners and then remove from corners_to_pop
                if nextState in self.corners:# and nextState not in state[1]:#in self.have_visited:
                    if nextState in state[1]:
                        try:
                            have_not_visited.remove(nextState)
                        except ValueError:
                            print "There is no item to remove"
                            exit()
                have_not_visited=tuple(have_not_visited)
                successors.append( ((nextState, have_not_visited), action, f))
        self._expanded += 1 # DO NOT CHANGE
        # Bookkeeping for display purposes
...
def manhattan_general(position, goal):
...
def foodHeuristic(state, problem):
    ...
    for location in food_location:
        g=mazeDistance(position, location, problem.startingGameState)
        if g>my_max:
            my_max=g
    return my_max
